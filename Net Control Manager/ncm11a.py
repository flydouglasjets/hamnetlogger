...
from net_engine import WebSocketHostThread, WebSocketClientThread
            ...
            if isinstance(data, list):
                ...
                for item in data:
                    if isinstance(item, dict):
                        ...
                        out.append({"callsign": item.upper(), "name": ""})
                return out
            return []
...
def dispatch_api_status_update(api_cfg, status_text_or_flag, app_ref=None):
    ...
    def worker():
        ...
        qrz = api_cfg.get("qrz", "").strip().upper()
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if isinstance(main_window, NetControlApp):
                ...
                # Trigger network broadcast from main application
                main_window.trigger_id_timer_reset()
...
class NcsScribeLoginDialog(QDialog):
    def __init__(self, default_ncs="", station_types=None, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Net Control Station & Network Setup")
        ...
        # ---------------------------------------------------------------------
        # 1. Operating Role & Callsign Section
        # ---------------------------------------------------------------------
        oper_gb = QGroupBox("Operator Identity")
        oper_layout = QGridLayout(oper_gb)

        oper_layout.addWidget(QLabel("Select Operating Role:"), 0, 0)
        self.combo_role = QComboBox()
        self.combo_role.addItems(["NCS (Net Control Station)", "ANCS (Alternate Net Control)", "Scribe (Assistant)"])
        oper_layout.addWidget(self.combo_role, 0, 1)

        oper_layout.addWidget(QLabel("Operator Callsign:"), 1, 0)
        self.input_ncs = UppercaseLineEdit()
        self.input_ncs.setPlaceholderText("e.g. W9A")
        self.input_ncs.setText(default_ncs.upper())
        oper_layout.addWidget(self.input_ncs, 1, 1)

        layout.addWidget(oper_gb)

        # ---------------------------------------------------------------------
        # 2. Network Mode Selection Section
        # ---------------------------------------------------------------------
        net_gb = QGroupBox("Network Synchronization Mode")
        net_layout = QVBoxLayout(net_gb)

        net_layout.addWidget(QLabel("Select Connection Type:"))
        self.combo_sync_mode = QComboBox()
        self.combo_sync_mode.addItems([
            "Offline (Standalone Mode)",
            "Online (Host - Embedded Socket Server)",
            "Online (Guest - Join Remote Host)"
        ])
        self.combo_sync_mode.currentIndexChanged.connect(self.on_network_mode_changed)
        net_layout.addWidget(self.combo_sync_mode)

        # Host / Guest Configuration Grid
        self.net_config_container = QWidget()
        self.grid_conn = QGridLayout(self.net_config_container)
        self.grid_conn.setContentsMargins(0, 4, 0, 0)

        self.lbl_address = QLabel("Server IP / Hostname:")
        self.input_address = QLineEdit()
        self.input_address.setPlaceholderText("e.g. 192.168.1.50 or tailscale-ip")
        self.input_address.setText("127.0.0.1")

        self.lbl_port = QLabel("Port:")
        self.input_port = QLineEdit()
        self.input_port.setText("8765")
        self.input_port.setFixedWidth(80)

        self.grid_conn.addWidget(self.lbl_address, 0, 0)
        self.grid_conn.addWidget(self.input_address, 0, 1)
        self.grid_conn.addWidget(self.lbl_port, 1, 0)
        self.grid_conn.addWidget(self.input_port, 1, 1)

        self.lbl_hint = QLabel()
        self.lbl_hint.setStyleSheet("color: #000080; font-size: 8.5pt;")
        self.lbl_hint.setWordWrap(True)

        net_layout.addWidget(self.net_config_container)
        net_layout.addWidget(self.lbl_hint)

        layout.addWidget(net_gb)

        # ---------------------------------------------------------------------
        # 3. Local Scribe Section (Offline Only)
        # ---------------------------------------------------------------------
        self.scribe_gb = QGroupBox("Local Scribe Entry")
        scribe_gb_layout = QVBoxLayout(self.scribe_gb)

        self.chk_enable_scribe = EnterToggleCheckBox("Log in additional local Scribe Callsign")
        self.chk_enable_scribe.toggled.connect(self.toggle_scribe_container)
        scribe_gb_layout.addWidget(self.chk_enable_scribe)

        self.scribe_container = QWidget()
        scribe_layout = QGridLayout(self.scribe_container)
        scribe_layout.setContentsMargins(0, 4, 0, 0)

        scribe_layout.addWidget(QLabel("Scribe Callsign:"), 0, 0)
        self.input_scribe = UppercaseLineEdit()
        self.input_scribe.setPlaceholderText("e.g. K9B")
        scribe_layout.addWidget(self.input_scribe, 0, 1)

        scribe_layout.addWidget(QLabel("Station Type:"), 1, 0)
        ...
        scribe_gb_layout.addWidget(self.scribe_container)
        self.scribe_container.setVisible(False)

        layout.addWidget(self.scribe_gb)

        # ---------------------------------------------------------------------
        # Dialog Action Buttons
        # ---------------------------------------------------------------------
        btn_layout = QHBoxLayout()
        btn_confirm = EnterClickButton("Log In & Connect")
        ...
        self.on_network_mode_changed(0)

    def on_network_mode_changed(self, index):
        # 0: Offline, 1: Online (Host), 2: Online (Guest)
        if index == 0:
            self.net_config_container.setVisible(False)
            self.lbl_hint.setVisible(False)
            self.scribe_gb.setVisible(True)
        elif index == 1:
            self.net_config_container.setVisible(True)
            self.lbl_address.setEnabled(False)
            self.input_address.setEnabled(False)
            self.input_address.setText("0.0.0.0")
            self.lbl_hint.setVisible(True)
            self.lbl_hint.setText("Spins up an embedded WebSocket server. Scribe and auxiliary stations connect directly to your IP on port 8765.")
            self.scribe_gb.setVisible(False)
            self.chk_enable_scribe.setChecked(False)
        else:
            self.net_config_container.setVisible(True)
            self.lbl_address.setEnabled(True)
            self.input_address.setEnabled(True)
            if self.input_address.text() == "0.0.0.0":
                self.input_address.setText("127.0.0.1")
            self.lbl_hint.setVisible(True)
            self.lbl_hint.setText("Connects as a client machine to the Host station running the net server.")
            self.scribe_gb.setVisible(False)
            self.chk_enable_scribe.setChecked(False)
    ...
    def handle_accept(self):
        ncs_val = self.input_ncs.text().strip().upper()
        scribe_val = self.input_scribe.text().strip().upper()
    ...
    def get_logins(self):
        ...
        net_cfg = {
            "mode": "OFFLINE" if mode_idx == 0 else ("HOST" if mode_idx == 1 else "GUEST"),
            "address": self.input_address.text().strip() or "127.0.0.1",
            "port": port,
            "uri": f"ws://{self.input_address.text().strip() or '127.0.0.1'}:{port}"
        }

        return ncs, scribe, scribe_type, role_text, net_cfg


class NetConnectionDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Online Net Synchronization Setup")
        self.setFixedSize(420, 260)

        layout = QVBoxLayout(self)

        lbl_mode = QLabel("Select Synchronization Role:")
        lbl_mode.setFont(QFont("Tahoma", 9, QFont.Weight.Bold))
        layout.addWidget(lbl_mode)

        self.combo_sync_mode = QComboBox()
        self.combo_sync_mode.addItems([
            "Host Net (LAN / Embedded WebSocket Server)",
            "Join Existing Net (Client Connection)"
        ])
        self.combo_sync_mode.currentIndexChanged.connect(self.on_mode_changed)
        layout.addWidget(self.combo_sync_mode)

        self.grid_conn = QGridLayout()
        ...
        layout.addLayout(self.grid_conn)

        self.lbl_hint = QLabel("Hosting spins up an embedded socket server on port 8765.")
        self.lbl_hint.setStyleSheet("color: #000080; font-size: 8.5pt;")
        self.lbl_hint.setWordWrap(True)
        layout.addWidget(self.lbl_hint)

        btn_layout = QHBoxLayout()
        btn_connect = EnterClickButton("Connect & Start")
        btn_connect.setStyleSheet(BTN_GREEN_STYLE)
    ...
    def on_mode_changed(self, index):
        if index == 0:
            self.lbl_address.setEnabled(False)
            self.input_address.setEnabled(False)
            self.lbl_hint.setText("Hosting creates an embedded socket server. Scribes connect directly to your IP on the specified port.")
        else:
            self.lbl_address.setEnabled(True)
            self.input_address.setEnabled(True)
            self.lbl_hint.setText("Enter the IP or Tailscale/cloud domain of the host station.")

    def get_connection_config(self):
        is_host = (self.combo_sync_mode.currentIndex() == 0)
        address = self.input_address.text().strip() or "127.0.0.1"
        try:
            port = int(self.input_port.text().strip())
        except ValueError:
...
class CombinedStationSearchEditDialog(QDialog):
    ...
    def load_match_data(self):
        if 0 <= self.current_match_pos < len(self.matching_indices):
            ...
            self.edit_call.setText(st.get("callsign", "").upper())
    ...
    def save_current_station(self):
        ...
        old_call = st.get("callsign", "").upper()
        ...
        if self.parent() and hasattr(self.parent(), "broadcast_event"):
            # Send old_callsign along with updated dictionary
            payload = dict(st)
            payload["old_callsign"] = old_call
            self.parent().broadcast_event("UPDATE_STATION", payload)
...
class CombinedTrafficSearchEditDialog(QDialog):
    ...
    def execute_search(self):
        ...
        for idx, act in enumerate(self.activities):
            ...
            ts = act.get("timestamp", act.get("time", ""))
    ...
    def load_match_data(self):
        if 0 <= self.current_match_pos < len(self.matching_indices):
            ...
            self.edit_call.setText(act.get("callsign", act.get("source", "")).upper())
            self.edit_type.setCurrentText(act.get("type", act.get("condition", act.get("tag", ""))))
            self.edit_time.setText(act.get("timestamp", act.get("time", "")))
            if self.is_skywarn:
                self.edit_location.setText(act.get("location", "-"))
            self.edit_note.setText(act.get("note", act.get("narrative", act.get("details", ""))))
    ...
    def save_current_entry(self):
        if 0 <= self.current_match_pos < len(self.matching_indices):
            ...
            if self.is_skywarn:
                ...
                act["callsign"] = act["source"]
                act["condition"] = self.edit_type.currentText()
                act["type"] = act["condition"]
                ...
                act["details"] = act["narrative"]
            else:
                act["callsign"] = self.edit_call.text().strip().upper()
                act["source"] = act["callsign"]
                act["type"] = self.edit_type.currentText()
                act["tag"] = act["type"]
                act["timestamp"] = self.edit_time.text().strip()
                act["time"] = act["timestamp"]
                act["note"] = self.edit_note.toPlainText().strip()
                act["details"] = act["note"]

            if self.parent() and hasattr(self.parent(), "broadcast_event"):
                self.parent().broadcast_event("UPDATE_TRAFFIC", {"index": idx, "entry": act})
...
class UserManagementDialog(QDialog):
    def __init__(self, users_data, parent=None):
        ...
        for user in self.users:
            ...
            call_item = QTableWidgetItem(user.get("callsign", "").upper())
...
class ApiConfigDialog(QDialog):
    def __init__(self, api_data, web_opts_data, mode_text="Regular", parent=None):
        ...
        self.input_qrz.setPlaceholderText("e.g. W9A")
        self.input_qrz.setText(self.api_data.get("qrz", "").upper())
...
class NtsRadiogramDialog(QDialog):
    def __init__(self, default_station="", parent=None):
        ...
        self.input_origin.setText(default_station.upper())
    ...
    def clear_all_fields(self):
        ...
        self.combo_precedence.setCurrentIndex(0)
        self.input_origin.clear()
        self.input_check.clear()
        self.input_place.clear()
        self.input_time_filed.setText(datetime.now().strftime("%H:%M"))
        self.input_date.setText(datetime.now().strftime("%b %d"))
        self.combo_hx.setCurrentIndex(0)
        self.input_hx_val.clear()

        self.input_to_name.clear()
        self.input_to_phone.clear()
        self.input_to_addr.clear()
        self.input_to_email.clear()
        self.input_to_csz.clear()

        self.input_rx_station.clear()
        self.input_rx_name.clear()
        self.input_rx_street.clear()
        self.input_rx_csz.clear()
        self.input_rx_contact.clear()

        self.text_body.clear()

        self.input_recd_from.clear()
        self.input_recd_date.clear()
        self.input_recd_time.clear()
        self.input_sent_to.clear()
        self.input_sent_date.clear()
        self.input_sent_time.clear()
...
class ProfileEditorDialog(QDialog):
    ...
    def load_selected_profile(self):
        ...
        if "users" in auth_cfg:
            ...
            self.temp_auth_users = [{"callsign": c.upper(), "passcode": passc} for c in calls] if calls else [{"callsign": "", "passcode": passc}]
...
class RollCallDialog(QDialog):
    ...
    def is_already_checked_in(self, callsign):
        if self.main_app and isinstance(self.main_app, NetControlApp):
            return any(st["callsign"] == callsign.upper() for st in self.main_app.stations)
        return False

    def seek_next_valid(self, forward=True):
        while 0 <= self.index < len(self.roster):
            call = self.roster[self.index]["callsign"].upper()
    ...
    def update_display(self):
        if 0 <= self.index < len(self.roster):
            ...
            self.lbl_callsign.setText(st["callsign"].upper())
    ...
    def check_in_current(self):
        if 0 <= self.index < len(self.roster):
            ...
            if self.main_app and isinstance(self.main_app, NetControlApp):
                self.main_app.add_rollcall_checkin(st["callsign"].upper(), col_detail, st["name"])
...
class NetControlApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.btn_online_sync = QPushButton("Mode: Offline\n[Click to Connect]")
        self.btn_online_sync.setFixedSize(130, 38)
        self.btn_online_sync.clicked.connect(self.toggle_online_mode)
        ...
        footer_layout.addWidget(self.btn_online_sync)
    ...
    def rebuild_ui_for_mode(self, mode_name, traffic_types, net_statuses, has_roll_call=False, use_custom_statuses=False):
        ...
        if mode_name == "SKYWARN":
            ...
            self.traffic_group.setTitle("Net Traffic")
            self.activity_group.setTitle("Traffic and Activity Log")
    ...
    # -------------------------------------------------------------------------
    # Network Synchronization Operations
    # -------------------------------------------------------------------------

    def toggle_online_mode(self):
        active_thread = getattr(self, "host_thread", None) or getattr(self, "net_thread", None)
        if active_thread and active_thread.isRunning():
            active_thread.stop()
            active_thread.wait()
            self.host_thread = None
            self.net_thread = None
            self.btn_online_sync.setText("Mode: Offline\n[Click to Connect]")
            self.btn_online_sync.setStyleSheet("")
            self.add_activity_log(
                self.get_active_logging_callsign(), "NETWORK", "-", "=== DISCONNECTED / REVERTED TO OFFLINE ==="
            )
            return

        dialog = NetConnectionDialog(parent=self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            cfg = dialog.get_connection_config()

            if cfg["is_host"]:
                self.host_thread = WebSocketHostThread(host="0.0.0.0", port=cfg["port"])
                self.host_thread.get_current_state_callback = self.get_roster_snapshot
                self.host_thread.message_received.connect(self.handle_network_packet)
                self.host_thread.start()

                self.btn_online_sync.setText(f"ONLINE HOST\nPort {cfg['port']}")
                self.btn_online_sync.setStyleSheet(BTN_GREEN_STYLE)
                self.add_activity_log(
                    self.get_active_logging_callsign(), "NETWORK", "-", f"=== SOCKET SERVER HOSTED ON PORT {cfg['port']} ==="
                )
            else:
                self.btn_online_sync.setText("ONLINE JOIN\nConnecting...")
                self.btn_online_sync.setStyleSheet(BTN_CYAN_STYLE)

                self.net_thread = WebSocketClientThread(uri=cfg["uri"], parent=self)
                self.net_thread.message_received.connect(self.handle_network_packet)
                self.net_thread.connected.connect(self.on_client_connected)
                self.net_thread.disconnected.connect(self.on_client_disconnected)
                self.net_thread.start()

    def get_roster_snapshot(self):
        return {
            "action": "FULL_SYNC",
            "sender": self.get_active_logging_callsign(),
            "payload": {
    ...
    def on_client_connected(self):
        self.btn_online_sync.setText("ONLINE JOINED\nConnected")
        self.btn_online_sync.setStyleSheet(BTN_CYAN_STYLE)
        self.add_activity_log(
            self.get_active_logging_callsign(), "NETWORK", "-", "=== CONNECTED TO HOST SOCKET ==="
        )
        for st in self.stations:
            self.broadcast_event("CHECKIN", st)

    def on_client_disconnected(self, err_msg):
        self.btn_online_sync.setText("Mode: Offline\n[Connect Failed]")
        self.btn_online_sync.setStyleSheet(BTN_DANGER_STYLE)
        QMessageBox.warning(self, "Network Connection Failed", f"Could not connect to WebSocket server:\n{err_msg}")

    def broadcast_event(self, action, payload):
        active_thread = getattr(self, "host_thread", None) or getattr(self, "net_thread", None)
        if active_thread and active_thread.isRunning():
            packet = {
                "action": action,
            ...
            active_thread.send_data(packet)

    def handle_network_packet(self, packet):
        action = packet.get("action")
        sender = packet.get("sender", "REMOTE")
        payload = packet.get("payload", {})

        if action == "FULL_SYNC":
            if "stations" in payload:
                for remote_st in payload["stations"]:
                    self.add_remote_checkin(remote_st)
            if "activities" in payload:
                for remote_act in payload["activities"]:
                    if remote_act not in self.activities:
                        self.activities.append(remote_act)
                self.refresh_activity_table_display()

        elif action == "CHECKIN":
            self.add_remote_checkin(payload)

        elif action == "UPDATE_STATION":
            target_call = payload.get("callsign", "").upper()
            old_call = payload.get("old_callsign", target_call).upper()
            
            updated = False
            for i, st in enumerate(self.stations):
                # Match on target callsign OR old callsign if callsign was modified
                if st.get("callsign", "").upper() in (target_call, old_call):
                    self.stations[i] = payload
                    updated = True
                    break
            
            if not updated:
                self.stations.append(payload)
            ...
            target_call = payload.get("callsign", "").upper()
            self.stations = [st for st in self.stations if st.get("callsign", "").upper() != target_call]
            self.refresh_station_table()
            self.refresh_callsign_combo()

        elif action == "NET_STATE_CHANGE":
            self.is_net_active = payload.get("is_net_active", False)
            self.net_state_name = payload.get("net_state_name", "INACTIVE")
            ...
            st_name = payload.get("status")
            if st_name and st_name in self.net_statuses:
                val = self.net_statuses[st_name]
                t_hex, b_hex = parse_color_map(val)
                self.status_badge.set_state(st_name, t_hex, b_hex)

        elif action == "TRAFFIC_LOG":
            self.add_remote_traffic_log(sender, payload)

        elif action == "UPDATE_TRAFFIC":
            index = payload.get("index")
            entry = payload.get("entry")
            if index is not None and 0 <= index < len(self.activities):
                self.activities[index] = entry
            else:
                ts = entry.get("timestamp", entry.get("time", ""))
                c_call = entry.get("callsign", entry.get("source", "")).upper()
                for i, act in enumerate(self.activities):
                    act_ts = act.get("timestamp", act.get("time", ""))
                    act_call = act.get("callsign", act.get("source", "")).upper()
                    if act_ts == ts and act_call == c_call:
                        self.activities[i] = entry
                        break
            self.refresh_activity_table_display()

        elif action == "DELETE_TRAFFIC":
            target_call = payload.get("callsign", payload.get("source", "")).upper()
            target_ts = payload.get("timestamp", payload.get("time", ""))
            self.activities = [
                act for act in self.activities
                if not (
                    act.get("callsign", act.get("source", "")).upper() == target_call
                    and act.get("timestamp", act.get("time", "")) == target_ts
                )
            ]
        elif action == "RESET_ID_TIMER":
            # Synchronize timer reset across all client/host nodes
            is_active = payload.get("is_net_active", self.is_net_active)
            self.id_timer_label.reset_timer(is_net_active=is_active)
    ...
    def on_table_cell_changed(self, item):
        ...
        try:
            ...
            payload = dict(st)
            payload["old_callsign"] = old_call
            self.broadcast_event("UPDATE_STATION", payload)
        finally:
    ...
    def on_activity_cell_changed(self, item):
        ...
        try:
            ...
            self.broadcast_event("UPDATE_TRAFFIC", {"index": row, "entry": act})
        finally:
            self.activity_table.blockSignals(False)

    def add_remote_checkin(self, st_data):
        call = st_data.get("callsign", "").upper()
        if not call:
            return
        st_data["callsign"] = call
        ...
        self.register_station_callsign(call, st_data.get("info", st_data.get("detail", "")))
        self.refresh_station_table()
        self.refresh_callsign_combo()
        self.station_table.scrollToBottom()

    def add_remote_traffic_log(self, sender, payload):
        callsign = payload.get("callsign", payload.get("call", sender)).upper()
        tag = payload.get("tag", payload.get("type", "NETWORK"))
        details = payload.get("details", payload.get("note", ""))
    ...
    def trigger_id_timer_reset(self):
        """Resets local timer and broadcasts the reset event to all connected peers."""
        self.id_timer_label.reset_timer(is_net_active=self.is_net_active)
        self.broadcast_event("RESET_ID_TIMER", {
    ...
    def prompt_ncs_login(self):
        ...
        while True:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ncs_call, scribe_call, scribe_type, role, net_cfg = dialog.get_logins()
                self.current_ncs_callsign = ncs_call.upper()
                self.current_scribe_callsign = scribe_call.upper() if scribe_call else ""
                self.current_user_role = role
                self.ncs_badge.set_ncs(self.current_ncs_callsign, self.current_scribe_callsign)

                if "api_sync" in prof:
                    prof["api_sync"]["qrz"] = self.current_ncs_callsign

                self.auto_add_ncs_to_roster(self.current_ncs_callsign)
                if self.current_scribe_callsign:
                    self.auto_add_scribe_to_roster(self.current_scribe_callsign, station_type=scribe_type)

                # Initialize network connection state based on dialog configuration
                self.init_network_from_login(net_cfg)
                return True
            else:
                reply = QMessageBox.question(
                    self,
    ...
    def init_network_from_login(self, net_cfg):
        # Clean up existing socket threads if re-logging in
        active_thread = getattr(self, "host_thread", None) or getattr(self, "net_thread", None)
        if active_thread and active_thread.isRunning():
            active_thread.stop()
            active_thread.wait()
        ...
        if mode == "OFFLINE":
            ...
            self.host_thread = WebSocketHostThread(host="0.0.0.0", port=net_cfg["port"])
            self.host_thread.get_current_state_callback = self.get_roster_snapshot
            self.host_thread.message_received.connect(self.handle_network_packet)
            self.host_thread.start()
            ...
            self.btn_online_sync.setStyleSheet(BTN_GREEN_STYLE)
            self.add_activity_log(
                self.get_active_logging_callsign(), "NETWORK", "-", f"=== SOCKET SERVER HOSTED ON PORT {net_cfg['port']} ==="
            )
        elif mode == "GUEST":
            self.btn_online_sync.setText("ONLINE JOIN\nConnecting...")
            self.btn_online_sync.setStyleSheet(BTN_CYAN_STYLE)

            self.net_thread = WebSocketClientThread(uri=net_cfg["uri"], parent=self)
            self.net_thread.message_received.connect(self.handle_network_packet)
            self.net_thread.connected.connect(self.on_client_connected)
            self.net_thread.disconnected.connect(self.on_client_disconnected)
            self.net_thread.start()
    ...
    def get_active_logging_callsign(self):
        if getattr(self, "current_scribe_callsign", ""):
            return self.current_scribe_callsign.upper()
        return self.current_ncs_callsign.upper() if self.current_ncs_callsign else "NET CONTROL"

    # -------------------------------------------------------------------------
    # Net Operations & Logging Controls
    # -------------------------------------------------------------------------
    ...
    def toggle_net_mode(self):
        ...
        # BROADCAST NET START/CLOSE STATE TO PEERS
        self.broadcast_event("NET_STATE_CHANGE", {
            "is_net_active": self.is_net_active,
    ...
    def apply_selected_net_status(self):
        ...
        # BROADCAST CUSTOM STATUS CHANGE TO PEERS
        self.broadcast_event("STATUS_CHANGE", {"status": st_name})
    ...
    def checkin_station(self):
        ...
        call = raw_call.split("—")[0].strip().upper()
        ...
        for st in self.stations:
            if st["callsign"].upper() == call:
                ...
                if confirm == QMessageBox.StandardButton.Yes:
                    ...
                    self.station_table.scrollToBottom()
                    self.broadcast_event("UPDATE_STATION", st)
                return
        ...
        self.clear_checkin_inputs()
        self.station_table.scrollToBottom()
        self.broadcast_event("CHECKIN", station_data)
    ...
    def add_rollcall_checkin(self, callsign, detail, info_name):
        call = callsign.strip().upper()
        self.register_station_callsign(call, info_name)
        now_str = datetime.now().strftime("%H:%M")
        ...
        self.refresh_callsign_combo()
        self.station_table.scrollToBottom()
        self.broadcast_event("CHECKIN", st_data)
    ...
    def delete_station(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            deleted_st = self.stations.pop(selected)
            self.refresh_station_table()
            self.refresh_callsign_combo()
            self.broadcast_event("DELETE_STATION", {"callsign": deleted_st.get("callsign")})
    ...
    def clear_station_roster(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            for st in list(self.stations):
                self.broadcast_event("DELETE_STATION", {"callsign": st.get("callsign")})
            self.stations.clear()
            self.refresh_station_table()
            self.refresh_callsign_combo()
    ...
    def refresh_station_table(self):
        ...
        try:
            for row, st in enumerate(self.stations):
                ...
                if self.current_mode == "SKYWARN":
                    items = [
                        QTableWidgetItem(st["callsign"].upper()),
    ...
    def refresh_callsign_combo(self):
        current = self.combo_traffic_call.currentText().upper()
        self.combo_traffic_call.clear()
        self.combo_traffic_call.addItem("")
        for st in self.stations:
            if st["status"] != "Checked Out":
                self.combo_traffic_call.addItem(st["callsign"].upper())
    ...
    # -------------------------------------------------------------------------
    # Traffic & Activity Log Operations
    # -------------------------------------------------------------------------
    ...
    def add_activity_log(self, callsign, tag, state_str, details, timestamp=None, is_remote=False):
        if not timestamp:
        ...
        if not is_remote:
            self.broadcast_event("TRAFFIC_LOG", entry)
    ...
    def delete_traffic_entry(self):
        ...
        call = act.get("callsign", act.get("source", "")).upper()
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            deleted_act = self.activities.pop(selected)
            self.refresh_activity_table_display()
            self.broadcast_event("DELETE_TRAFFIC", deleted_act)
    ...
    def clear_activity_log(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            for act in list(self.activities):
                self.broadcast_event("DELETE_TRAFFIC", act)
            self.activities.clear()
            self.activity_table.setRowCount(0)
    ...
    def refresh_activity_table_display(self):
        ...
        try:
            ...
            for act in self.activities:
                ...
                if self.current_mode == "SKYWARN":
                    self.activity_table.setItem(row, 0, QTableWidgetItem(act.get("timestamp", act.get("time", ""))))
                    ...
                    self.activity_table.setItem(row, 3, QTableWidgetItem(act.get("source", act.get("callsign", "")).upper()))
                    self.activity_table.setItem(row, 4, QTableWidgetItem(act.get("narrative", act.get("note", act.get("details", "")))))
                else:
                    self.activity_table.setItem(row, 0, QTableWidgetItem(act.get("timestamp", act.get("time", ""))))
                    self.activity_table.setItem(row, 1, QTableWidgetItem(act.get("callsign", act.get("source", "")).upper()))

                    type_str = act.get("type", act.get("tag", ""))
                    type_item = QTableWidgetItem(type_str)
                    if type_str == "Emergency":
                    ...
                    self.activity_table.setItem(row, 3, QTableWidgetItem(act.get("note", act.get("details", ""))))
        finally:
            self.activity_table.blockSignals(False)
    ...
    def open_nts_dialog(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            self.add_activity_log(
                callsign=data["origin"].upper(),
                tag="Priority" if data["precedence"] in ("P", "E") else "General Note",
                state_str=data["place"],
    ...
    def load_session_from_dict(self, file_path):
        try:
            ...
            self.current_ncs_callsign = data.get("current_ncs_callsign", "UNASSIGNED").upper()
            self.current_scribe_callsign = data.get("current_scribe_callsign", "").upper()
            self.ncs_badge.set_ncs(self.current_ncs_callsign, self.current_scribe_callsign)

            self.stations = data.get("stations", [])
            for st in self.stations:
                if "callsign" in st:
                    st["callsign"] = st["callsign"].upper()

            self.activities = data.get("activities", [])
            for act in self.activities:
                if "callsign" in act:
                    act["callsign"] = act["callsign"].upper()
                if "source" in act:
                    act["source"] = act["source"].upper()
        ...
        try:
            with open(path, mode="w", newline="", encoding="utf-8") as f:
                ...
                if self.current_mode == "SKYWARN":
                    ...
                    for st in self.stations:
                        writer.writerow([st.get("callsign", "").upper(), st.get("type", ""), st.get("detail", "-"), st.get("status", ""), st.get("time_in", ""), st.get("time_out", "")])
                else:
                    writer.writerow(["Call", "Station Type", "Additional", "Info / Grid", "Status", "In", "Out"])
                    for st in self.stations:
                        writer.writerow([st.get("callsign", "").upper(), st.get("type", ""), st.get("detail", "-"), st.get("info", "-"), st.get("status", ""), st.get("time_out", "")])
                ...
                if self.current_mode == "SKYWARN":
                    ...
                    for act in self.activities:
                        writer.writerow([act.get("timestamp", act.get("time", "")), act.get("condition", ""), act.get("location", ""), act.get("source", "").upper(), act.get("narrative", "")])
                else:
                    writer.writerow(["Time", "Callsign", "Traffic Tag", "Details"])
                    for act in self.activities:
                        writer.writerow([act.get("timestamp", act.get("time", "")), act.get("callsign", "").upper(), act.get("type", ""), act.get("note", "")])
    ...
    def export_pdf(self):
        ...
        <body>
            ...
            <div class="meta">Generated: {date_str} | Profile: {self.current_profile_name} ({self.current_mode} Mode) | NCS: {self.current_ncs_callsign.upper()}</div>
        ...
        for st in self.stations:
            ...
            html += f"""
                <tr{row_class}>
                    <td><b>{st.get('callsign', '').upper()}</b></td>
                    <td>{st.get('type', '')}</td>
                    <td>{st.get('detail', '-')}</td>
                    {info_td}
                    <td>{st.get('status', '')}</td>
        ...
        for act in self.activities:
            if self.current_mode == "SKYWARN":
                html += f"""
                    <tr>
                        <td>{act.get('timestamp', act.get('time', ''))}</td>
                        <td><b>{act.get('condition', '')}</b></td>
                        <td>{act.get('location', '-')}</td>
                        <td>{act.get('source', '').upper()}</td>
                ...
                html += f"""
                    <tr>
                        <td>{act.get('timestamp', act.get('time', ''))}</td>
                        <td><b>{act.get('callsign', '').upper()}</b></td>
                        <td>{act.get('type', '')}</td>
                        <td>{act.get('note', '')}</td>
                    </tr>
                """
