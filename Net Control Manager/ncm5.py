...
STATION_MEMORY_FILE = "station_memory.json"
...
def save_station_memory(memory_list):
    try:
        with open(STATION_MEMORY_FILE, "w", encoding="utf-8") as f:
            json.dump(list(set(memory_list)), f, indent=2)
    except Exception as e:
    ...
    def worker():
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=6) as resp:
                ...
                if app_ref:
                    app_ref.add_activity_log("API SYNC", "HTTP POST", "-", f"Posted {param}={post_val} (HTTP {status_code})")
        except Exception as e:
            if app_ref:
...
class WebApiStatusBadgeLabel(QLabel):
    ...
    def set_status(self, text_val):
        clean_text = str(text_val).strip().upper()
        if not clean_text:
            clean_text = "N/A"
...
class NcsBadgeLabel(QLabel):
    ...
    def set_ncs(self, callsign):
        disp = callsign.upper() if callsign else "UNASSIGNED"
...
class ProfileSelectDialog(QDialog):
    def __init__(self, profiles, current_profile="", parent=None):
        ...
        for mode_key, lwidget in self.list_widgets.items():
            ...
            lwidget.itemDoubleClicked.connect(lambda item: self.finish_selection(item.text()))
    ...
    def accept_selection(self):
        for lwidget in self.list_widgets.values():
            ...
            if selected:
                self.selected_profile = selected[0].text()
                self.accept()
                return
...
class ProfileEditorDialog(QDialog):
    def __init__(self, profiles, current_profile="", parent=None):
        ...
        self.input_prof_name.setFixedWidth(240)
        ...
        mode_row.addWidget(self.chk_has_roll_call)
        mode_row.addWidget(self.chk_custom_statuses)
        ...
        station_group.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        st_layout = QVBoxLayout(station_group)
        self.text_station_types = QTextEdit()
        self.text_station_types.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        ...
        traffic_group.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        tr_layout = QVBoxLayout(traffic_group)
        self.text_traffic_types = QTextEdit()
        self.text_traffic_types.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        tr_layout.addWidget(self.text_traffic_types)
        types_row.addWidget(traffic_group, 1)

        layout.addLayout(types_row, 1)

        self.statuses_group = QGroupBox("Net Statuses Color Map (Hex Code e.g. #FF0000 or #FFFF00)")
        self.statuses_group.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        statuses_layout = QVBoxLayout(self.statuses_group)

        self.table_statuses = QTableWidget(0, 3)
        self.table_statuses.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        ...
        statuses_layout.addWidget(self.table_statuses, 1)
        statuses_layout.addLayout(st_btns)
        layout.addWidget(self.statuses_group, 1)
        ...
        self.chk_enable_api.toggled.connect(self.toggle_api_fields)
        api_layout.addWidget(self.chk_enable_api)

        self.api_options_container = QWidget()
        api_options_layout = QVBoxLayout(self.api_options_container)
        api_options_layout.setContentsMargins(0, 0, 0, 0)

        api_grid = QHBoxLayout()

        col1 = QVBoxLayout()
        col1.addWidget(QLabel("Endpoint URL:"))
        self.input_api_url = QLineEdit()
        self.input_api_url.setPlaceholderText("https://example.org/api/doupdate/")
        col1.addWidget(self.input_api_url)

        col1.addWidget(QLabel("Default NCS Callsign (qrz parameter):"))
        self.input_api_qrz = UppercaseLineEdit()
        self.input_api_qrz.setPlaceholderText("e.g. W9A")
        col1.addWidget(self.input_api_qrz)
        api_grid.addLayout(col1)

        col2 = QVBoxLayout()
        col2.addWidget(QLabel("Status Field Name:"))
        self.input_api_param = QLineEdit()
        self.input_api_param.setPlaceholderText("mcinaresstatus or skywarnstatus")
        col2.addWidget(self.input_api_param)

        status_map_row = QHBoxLayout()
        v_act = QVBoxLayout()
        v_act.addWidget(QLabel("Active Web Text:"))
        self.input_api_active_val = QLineEdit()
        self.input_api_active_val.setPlaceholderText("e.g. ACTIVE or LEVEL 1")
        v_act.addWidget(self.input_api_active_val)

        v_inact = QVBoxLayout()
        v_inact.addWidget(QLabel("Inactive Web Text:"))
        self.input_api_inactive_val = QLineEdit()
        self.input_api_inactive_val.setPlaceholderText("e.g. INACTIVE or STANDBY")
        v_inact.addWidget(self.input_api_inactive_val)

        status_map_row.addLayout(v_act)
        status_map_row.addLayout(v_inact)
        col2.addLayout(status_map_row)

        api_grid.addLayout(col2)

        web_opts_vbox = QVBoxLayout()
        web_opts_vbox.addWidget(QLabel("Custom Web Status Dropdown Options (One per line):"))
        self.text_web_status_options = QTextEdit()
        self.text_web_status_options.setFixedHeight(75)
        self.text_web_status_options.setPlaceholderText("e.g.\nACTIVE\nSTANDBY\nCLOSED\n-Normal Operations-")
        web_opts_vbox.addWidget(self.text_web_status_options)

        api_options_layout.addLayout(api_grid)
        api_options_layout.addLayout(web_opts_vbox)

        api_layout.addWidget(self.api_options_container)
        layout.addWidget(self.api_group)

        scripts_group = QGroupBox("Script Tabs Management (Press Shift+Enter for line breaks)")
        scripts_group.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        ...
        layout.addWidget(scripts_group, 1)
    ...
    def toggle_api_fields(self, enabled):
        self.api_options_container.setVisible(enabled)
    ...
    def load_selected_profile(self):
        ...
        self.text_web_status_options.setText("\n".join(prof.get("web_status_options", ["ACTIVE", "STANDBY", "CLOSED"])))
        ...
        self.input_api_url.setText(api_cfg.get("url", "https://www.mcinares.org/chatgemilaude/doupdate/"))
        self.input_api_qrz.setText(api_cfg.get("qrz", "W9A"))
        self.input_api_param.setText(api_cfg.get("param_name", "skywarnstatus" if mode == "SKYWARN" else "mcinaresstatus"))
        self.input_api_active_val.setText(api_cfg.get("active_value", "ACTIVE"))
        self.input_api_inactive_val.setText(api_cfg.get("inactive_value", "INACTIVE"))
        self.toggle_api_fields(is_api_enabled)
        ...
        for row, sc in enumerate(scripts):
            ...
            self.table_scripts.setRowHeight(row, 80)
    ...
    def sync_form_to_data(self, target_name):
        ...
        web_opts = [w.strip() for w in self.text_web_status_options.toPlainText().split("\n") if w.strip()]
        ...
        self.profiles[target_name] = {
            ...
            "web_status_options": web_opts,
            "net_statuses": statuses_map,
            "scripts": scripts,
            "api_sync": {
                "enabled": self.chk_enable_api.isChecked(),
                "url": self.input_api_url.text().strip(),
                "qrz": self.input_api_qrz.text().strip().upper(),
                "param_name": self.input_api_param.text().strip(),
                "active_value": self.input_api_active_val.text().strip(),
                "inactive_value": self.input_api_inactive_val.text().strip()
            }
        }
    ...
    for p_col, s_col, n_col in column_blocks:
        for row in range(10, sheet.max_row + 1):
            ...
            if prefix and suffix:
                ...
                roster.append({"callsign": callsign, "name": op_name})
...
class RollCallDialog(QDialog):
    def __init__(self, roster, parent=None):
        ...
        legend_frame = QFrame()
        legend_frame.setFrameShape(QFrame.Shape.Panel)
        legend_frame.setFrameShadow(QFrame.Shadow.Sunken)
        legend_layout = QHBoxLayout(legend_frame)
        legend_layout.setContentsMargins(4, 2, 4, 2)

        legend_label = QLabel("[Ctrl+Space] In  |  [Ctrl+Down] Skip  |  [Ctrl+Up] Prev")
        legend_label.setStyleSheet("color: #000080; font-weight: bold;")
...
class NetControlApp(QMainWindow):
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=5) as resp:
                    ...
                    try:
                        ...
                        remote_val = str(data.get(param, "")).upper()
                    except Exception:
                        remote_val = raw_data.strip().upper()

                    if remote_val:
                        self.web_status_badge.set_status(remote_val)
            except Exception as e:
                self.web_status_badge.set_status("ERR/OFFLINE")
    ...
    def setup_shortcuts(self):
        ...
        self.shortcut_toggle_legends = QShortcut(QKeySequence("Ctrl+F12"), self)
    ...
    def init_ui(self):
        ...
        self.traffic_legend_lbl1 = QLabel("[F4] Focus Station Field  |  [Ctrl+Ent] Log Activity  |  [F12] Clear Form")
        ...
        self.completer = QCompleter(self.station_memory)
        ...
        self.legend_label1 = QLabel("[F2] Add  |  [F3] Search  |  [F8] Roll Call")
        self.legend_label1.setStyleSheet("color: #000080;")
        self.legend_label1.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.legend_label2 = QLabel("[Ctrl+E] Edit  |  [Ctrl+T] Standby  |  [Ctrl+U] Out")
        self.legend_label2.setStyleSheet("color: #000080;")
        self.legend_label2.setAlignment(Qt.AlignmentFlag.AlignCenter)

        legend_layout.addWidget(self.legend_label1)
        legend_layout.addWidget(self.legend_label2)
        ...
        btn_traffic_search = QPushButton("Search\nRoster")
        btn_traffic_search.clicked.connect(self.open_traffic_search_dialog)

        btn_traffic_edit = QPushButton("Edit\nDetails")
        ...
        activity_legend_label = QLabel("[F5] Search Log  |  [F9/F10] Web Status  |  [Shift+F11/F11] Transfer Net")
        ...
        self.footer_legend_lbl1 = QLabel("[Ctrl+F12] Hide Legends  |  [F6] Select Profile  |  [F7] Create Profile  |  [Ctrl+S] Save Session")
    ...
    def transfer_net_control(self):
        ...
        if confirm == QMessageBox.StandardButton.Yes:
            ...
            self.ncs_badge.set_ncs(new_ncs)
    ...
    def register_station_callsign(self, callsign):
        call = callsign.strip().upper()
        if call and call not in self.station_memory:
            self.station_memory.append(call)
            save_station_memory(self.station_memory)
            self.completer.model().setStringList(self.station_memory)

    def add_rollcall_checkin(self, callsign, detail, info_name):
        self.register_station_callsign(callsign)
    ...
    def toggle_legends_visibility(self):
        ...
        if self.legends_visible:
            self.footer_legend_lbl1.setText("[Ctrl+F12] Hide Legends  |  [F6] Select Profile  |  [F7] Create Profile  |  [Ctrl+S] Save Session")
            ...
            self.footer_legend_lbl1.setText("[Ctrl+F12] Display Legends")
    ...
    def rebuild_ui_for_mode(self, mode_name, traffic_types, net_statuses, has_roll_call=False, use_custom_statuses=False):
        ...
        if has_roll_call:
            self.legend_label1.setText("[F2] Add  |  [F3] Search  |  [F8] Roll Call")
        else:
            self.legend_label1.setText("[F2] Add Check-in  |  [F3] Search Roster")
        ...
        self.traffic_legend_lbl2.setVisible(show_status_controls)
    ...
    def prompt_ncs_login(self):
        ...
        dialog = NcsLoginDialog(default_call=default_call, parent=self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ncs_call = dialog.get_ncs_callsign()
            self.current_ncs_callsign = ncs_call
            self.ncs_badge.set_ncs(ncs_call)
    ...
    def edit_station_details(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            self.register_station_callsign(call)
            self.refresh_station_table()
            self.refresh_callsign_combo()
    ...
    def delete_traffic_entry(self):
        selected = self.activity_table.currentRow()
        if selected < 0 or selected >= len(self.activities):
            QMessageBox.information(self, "Edit Traffic", "Please select a traffic entry to delete.")
            return
    ...
    def restore_session(self):
        ...
        try:
            ...
            self.ncs_badge.set_ncs(self.current_ncs_callsign)
