...
class CombinedStationSearchEditDialog(QDialog):
    ...
    def load_match_data(self):
        if 0 <= self.current_match_pos < len(self.matching_indices):
            ...
            # Ensure index safety if type isn't present in current list
    ...
    def __init__(self, parent=None):
        ...
        sections = [
            ...
            ("Roster Management", [
                ...
                ("F3 / Ctrl+E", "Search & Edit Roster Stations"),
            ...
            ("Log, Web API & NCS", [
                ("F5 / Shift+F5", "Search & Edit Activity Log"),
                ("Ctrl+Shift+E", "Search & Edit Traffic Log"),
...
class NetControlApp(QMainWindow):
    ...
    def setup_shortcuts(self):
        ...
        self.shortcut_search.activated.connect(self.open_combined_station_search_edit)

        self.shortcut_edit_station = QShortcut(QKeySequence("Ctrl+E"), self)
        self.shortcut_edit_station.activated.connect(self.open_combined_station_search_edit)
    ...
    def init_ui(self):
        ...
        btn_search_edit_station = QPushButton("Search & Edit\nRoster")
    ...
    def prompt_ncs_login(self):
        ...
        while True:
            dialog = NcsScribeLoginDialog(default_ncs=default_call, station_types=st_types, parent=self)
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ncs_call, scribe_call, scribe_type = dialog.get_logins()
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    continue  # Loop back to the login screen
                else:
                    return False  # User chose No, exit login flow
    ...
    def apply_profile(self, profile_name):
        ...
        # NOTE: self.prompt_ncs_login() removed from here to prevent double-prompting
        self.poll_remote_web_status()
        
    def select_profile_and_login(self):
        """Displays profile selector and forces NCS login. Loops if NCS login is declined."""
        while True:
            self.profiles = load_profiles_from_disk()
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted and dialog.selected_profile:
                ...
                if self.check_profile_access(target_prof):
                    ...
                    # Prompt for NCS/Scribe login. If successful, exit loop.
                    if self.prompt_ncs_login():
                        break
            else:
                # User canceled profile selection window
                break
    ...
    def reset_entire_net(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            # Reset NCS / Scribe identifiers
            self.current_ncs_callsign = "UNASSIGNED"
            self.current_scribe_callsign = ""
            self.ncs_badge.set_ncs("UNASSIGNED")

            # Prompt for NCS login; if canceled/declined, force profile selection
    ...
    def log_traffic_item(self):
        ...
        # Set focus back to the Station field for seamless logging
        self.combo_traffic_call.setFocus()
