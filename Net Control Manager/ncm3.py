...
class ProfileEditorDialog(QDialog):
    def __init__(self, profiles, current_profile="", parent=None):
        ...
        self.input_prof_name.setFixedWidth(300)
...
def parse_roll_call_file(file_path):
    ...
    # Read column block 1 top-to-bottom, then column block 2 top-to-bottom
...
class RollCallDialog(QDialog):
    """Interactive dialog to rapidly step through a pre-loaded roll call roster."""
    def __init__(self, roster, parent=None):
        ...
        legend_label = QLabel("[Ctrl+Space] Check In   |   [Ctrl+Down] Skip Station")
        ...
        # Explicit Window Shortcuts for flawless key handling
    ...
    def check_in_current(self):
        if self.index < len(self.roster):
            ...
            # Immediately add station to live main window roster
...
class NetControlApp(QMainWindow):
    ...
    def add_rollcall_checkin(self, callsign, detail, info_name):
        """Immediately checks in a station during roll call execution."""
        now_str = datetime.now().strftime("%H:%M")
        default_type = self.combo_station_type.itemText(0) if self.combo_station_type.count() > 0 else "Base"

        # Update if callsign already exists in roster
    ...
    def rebuild_ui_for_mode(self, mode_name, traffic_types, net_statuses, has_roll_call=False):
    ...
    def apply_profile(self, profile_name):
        ...
        self.rebuild_ui_for_mode(mode, tr_types, st_map, has_roll_call=has_rc)
