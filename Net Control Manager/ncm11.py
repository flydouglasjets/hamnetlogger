...
def toggle_online_mode(self):
        # If already connected, disconnect and revert to Offline
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if cfg["is_host"]:
                self.net_thread = WebSocketHostThread(host="0.0.0.0", port=cfg["port"], parent=self)
    ...
    def init_ui(self):
        ...
        left_subgroup.addWidget(self.btn_online_sync)
    ...
    def start_online_sync(self):
        ...
        if cfg["is_host"]:
            ...
            self.net_thread.start()
            self.add_activity_log(
                self.get_active_logging_callsign(),
                "NETWORK", "-", f"=== SOCKET SERVER HOSTED ON PORT {cfg['port']} ==="
            )
        else:
            self.net_thread = WebSocketClientThread(uri=cfg["uri"], parent=self)
            self.net_thread.message_received.connect(self.handle_network_packet)
            self.net_thread.connected.connect(lambda: self.add_activity_log(
                self.get_active_logging_callsign(), "NETWORK", "-", f"=== CONNECTED TO {cfg['uri']} ==="
            ))
            self.net_thread.disconnected.connect(lambda err: QMessageBox.warning(
                self, "Network Error", f"Disconnected from server:\n{err}"
            ))
            self.net_thread.start()

    def broadcast_event(self, action, payload):
        """Helper to send JSON packets over active thread."""
        if hasattr(self, "net_thread") and self.net_thread.isRunning():
            packet = {
                "action": action,
            ...
            self.net_thread.send_data(packet)

    def handle_network_packet(self, packet):
        """Dispatches incoming socket events from remote stations."""
        action = packet.get("action")
        sender = packet.get("sender", "REMOTE")
        payload = packet.get("payload", {})

        if action == "CHECKIN":
            # Add or update station locally without triggering another broadcast
            self.add_remote_checkin(payload)
        elif action == "TRAFFIC_LOG":
            # Append log item from another operator
    ...
    def add_remote_checkin(self, st_data):
        """Processes check-ins received from remote stations over WebSockets."""
        call = st_data.get("callsign")
        if not call:
            return
    ...
    def add_remote_traffic_log(self, sender, payload):
        """Processes traffic log entries received from remote stations."""
        time_str = payload.get("time", datetime.now().strftime("%H:%M"))
        call = payload.get("call", sender)
        traffic_type = payload.get("type", "General Note")
        note = payload.get("note", "")
        loc_val = payload.get("loc", "-")

        self.add_activity_log(call, traffic_type, loc_val, note, time_val=time_str)
    ...
    def log_traffic_item(self):
        ...
        self.input_traffic_note.clear()
        self.input_traffic_loc.clear()
        self.broadcast_event("TRAFFIC_LOG", {
            "call": call, "type": traffic_type, "note": note, "time": time_val, "loc": loc_val
            })
    ...
    def handle_network_packet(self, packet):
        ...
        if action == "CHECKIN":
            # Add station received from another operator without re-broadcasting
            self.add_remote_checkin(payload)
        elif action == "STATUS_CHANGE":
            # Sync status badge
            self.sync_remote_status(payload)
        elif action == "LOG_ENTRY":
            # Sync activity log
            self.add_remote_activity_log(payload)
